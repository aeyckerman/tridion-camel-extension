...
public class S3Producer extends DefaultProducer {
    ...
    public void process(final Exchange exchange) throws Exception {
        ...
        String actionType = exchange.getIn().getHeader(S3Constants.ACTION_TYPE, String.class);
        if (contentType != null) {
            objectMetadata.setContentType(contentType);
        }
        ...
        if (actionType.equals("delete")) {
            ...
            DeleteObjectRequest deleteObjectRequest = new DeleteObjectRequest(getConfiguration().getBucketName(), determineKey(exchange));

            LOG.trace("Put object [{}] from exchange [{}]...", deleteObjectRequest, exchange);

            getEndpoint().getS3Client().deleteObject(deleteObjectRequest);

            LOG.trace("Received result (only response code ?)");
